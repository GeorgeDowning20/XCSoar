...
\item[Show Restricted Areas]  Includes NOTAMs for airspace the pilot cannot
  legally enter: prohibited/restricted areas, activated danger areas,
  temporary restricted areas, military operating areas and airspace
  reservations.  These are drawn in red on the map instead of the generic
  grey NOTAM colour.  Enabled by default.
\item[Show Navigation Warnings]  Includes NOTAMs about navigation aids,
  ILS, GNSS and communication/radar facilities.  Enabled by default.
\item[Show En-Route Information]  Includes NOTAMs about airspace
  structure, ATS units and procedures (SIDs/STARs, approach procedures,
  etc.).  Enabled by default.
\item[Show Aerodrome NOTAMs]  Includes NOTAMs about aerodrome facilities,
  lighting and movement area (runways, taxiways, etc.).  Enabled by
  default.
\item[Show Obstacles]  Includes NOTAMs about new, changed or removed
  obstacles.  Enabled by default.
\item[Show Activity Warnings]  Includes advisory notices about hazardous
  activity that don't by themselves make the airspace illegal to enter:
  air displays, aerobatics, parachute jumping, UAS/drone operations, live
  firing or blasting exercises, model aircraft flying, etc.  These are
  not drawn in red, unlike Restricted Areas.  Enabled by default.
\end{description}
